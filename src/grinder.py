...
def grinder(fe: int, seed: int, runs: int,  instances_path: str, profiles: dict, output_path: str):
				...
				for i in range(runs):
					with ACO_MPSolver(instance, **profile) as solver:
						...
						for i, solution in enumerate(solver.run(iterations)):
							...
							yield instance, profile_name, profile, elapsed, i, iterations, solution, is_new_best
	...
	with open(output_file, 'w') as out:
		...
		for iteration in tqdm(runner()):
			instance, profile_name, profile, elapsed, i,total_iterations, solution, is_new_best = iteration
