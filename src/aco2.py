...
class ACOSolver(BaseSolver):
  ...
  def _run_single_ant(self, tau: np.ndarray, initial_position: int):
    ...
    if self.two_opt:
      cost = self._run_single_ants_two_opt(tours, cost)
  ...
  def _run_single_ants_two_opt(self, tours, cost):
  ...
  def _run_epoch(self, tau: np.ndarray, elitist_trail: np.ndarray):
    ...
    tau = (1-self.rho) * tau + epoch_trail_contribution + elitist_trail_contribution
